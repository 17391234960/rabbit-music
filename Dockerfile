...
RUN mvn -s /app/settings.xml -f /app/pom.xml clean package -Dmaven.test.skip=true -Dspring.profiles.active=prod
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/rabbit-music-api-0.0.1.jar .
